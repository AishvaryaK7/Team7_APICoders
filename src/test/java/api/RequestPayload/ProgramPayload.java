...
public class ProgramPayload {
	...
	public static JSONObject createRequestObjectForUpdate(Object[] row) {
		 request = new JSONObject();
		 request.put("programDescription", row[3]);
		 int rndInt = (int) Math.floor(Math.random() * (1000)+1);
		 request.put("programName", row[4]+ String.valueOf(rndInt));	 
		 request.put("programStatus", row[5]);
		 return request; 
	 }	
}